...
`Browser`侧通过选择指定的`LanguageModel`代理发起请求，经过`RPC`将请求发送到`Node`侧让实际的`LanguageModel`进行处理，最终在`Node`侧通过`RPC`将响应发送到`Browser`侧并消费该响应。基本示意图如下：

```mermaid
graph TD
  subgraph LanguageModelProxy[LanguageModel Proxy]
  end
